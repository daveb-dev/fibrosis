...
def test_snes_converge_to_tol():
    ...
    iterations (REGRESSION TEST) for Re = 200 '''

    pb = NSProblem('bfs.yaml')
...
def test_newton_converge_to_tol():
    ...
    number of iterations (REGRESSION TEST) for Re = 200 '''
...
def test_picard_converge_to_tol():
    ...
    pb.options['nonlinear']['atol'] = 1.e-09
    pb.init()
    picard_sol = NSSolver(pb)
    picard_sol.solve()

    assert len(picard_sol.residual) <= 44


def test_aitken_converge_to_tol():
    ''' test if Picard + Aitken solves Re = 200 case within 40 iterations '''
    pb = NSProblem('bfs.yaml')
    pb.options['nonlinear']['method'] = 'picard'
    pb.options['nonlinear']['atol'] = 1.e-09
    ...
    assert len(picard_sol.residual) <= 39
