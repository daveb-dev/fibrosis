...
def compute(prms):
    ...
    if prms['io']['xdmf'] == 1: # 
      sol = File(results_dir+"/minimal/sol.pvd")
      solh = File(results_dir+"/minimal/solh.pvd")
...
#      mesh_h, subdomains_h, boundaries_h = readmesh('./meshes/fibrosis/dummy_mesh.h5')
    ...
    theta_file  = './meshes/theta_functions_25x25_1_10.h5'
    mesh_file   = './meshes/random_fibro_25x25_1_10.h5'
    mesh_h_file = './meshes/homogenized_25x25.h5'
    ...
    TS      = TensorFunctionSpace(mesh_h, "CG", 1)
    ...
    # TODO: FIX THIS CODE!!!!! is just temporary
    Deff, C = generate_efective_tensor(io.prms_load('./input/prms_monodomain_homogenized.yaml'), subdomains, theta_c, theta_f, dofs, Deff, Deffp)
    ...
    est1h_, est2h_ = est1_intensity, est2_intensity
...
#    solverh.parameters['nonzero_initial_guess'] = True
    ...
    while t < T:
      ...
      if   (t >= est1_delay)*(t <= est1_delay + est1_duration):
        G		= assemble(g1*v *ds(2)  )
        Gh		= assemble(g1h*vh*ds_h(2))
      elif (t >= est2_delay)*(t <= est2_delay + est2_duration):
	    G		= assemble(g2*v *ds(1)  )
	    Gh		= assemble(g2h*vh*ds_h(1))
      else:
    	G, Gh = 0, 0;
      ...
      if counter == 10:      
          error_rel = evaluate_L2error(phi, phih)
