...
# Exact solution
    u_ex = Expression("x[0] < 0.5 ? \
            2/k0*x[0]*x[0] - (3*k0+k1-2*k0*k1)/(k0*(k0+k1))*x[0] : \
            2/k1*x[0]*x[0] - (3*k0+k1-2*k0*k1)/(k1*(k0+k1))*x[0] + \
            (k1*k1-k0*k1+k0-k1)/(k1*(k0+k1))", k0=k0, k1=k1, degree=2)
...
# interpolation of the discontinuity
    # kf = Expression("x[0]<=0.5 ? k0 : k1", k0=k0, k1=k1)
    # u = TrialFunction(V)
    # v = TestFunction(V)
...
# plot solutions
    ...
    e1.append(errornorm(u0, u1)/norm(u0))
    e2.append(errornorm(u0, u2)/norm(u0))
    e3.append(errornorm(u0, u3)/norm(u0))
